...
public class CrystalDataDataGenerator
{

    [MenuItem ("Data Generator/Data/Crystal")]
    static void GenerateJsonFromData ()
    {
        string csvFile = "Crystal Data.csv";
        ...
        AssetDataList<CrystalData> dataList = new AssetDataList<CrystalData> { data = new List<CrystalData> () };
        for (int i = 1; i < content.Length; i++)
        {
            ...
            string[] effectData = data[9].Split (",");
            ...
            CrystalData crystal = new CrystalData
            {
                ...
                unlock_rank = Parse.ToByte (data[4]),
                ...
                silhouette_id = data[12]
            };
            dataList.data.Add (crystal);
        }

        ZeroFormatterInitializer.Register ();

        dataPath += $"/{AssetDataFilename.crystal}";
        File.WriteAllBytes (dataPath, ZeroFormatterSerializer.Serialize (dataList));
        dataPath = $"{Application.streamingAssetsPath}/Data/Asset/CrystalData.json";
        File.WriteAllText (dataPath, JsonConvert.SerializeObject (dataList, Formatting.Indented));

        Debug.Log ($"<color=green>Asset data file \"{AssetDataFilename.crystal}\" has generated successful.</color>");
        Debug.Log (dataPath);
    }
}
