...
public class FairyFoodDataDataGenerator
{

    [MenuItem ("Data Generator/Data/Fairy Food")]
    static void GenerateJsonFromData ()
    {
        string csvFile = "Fairy Food Data.csv";
        ...
        for (int i = 1; i < content.Length; i++)
        {
            ...
            MaterialData fairyFood = new MaterialData
            {
                asset_id = data[0],
                reward_type = (Reward_Type)Parse.ToInt (data[2]),
                free_claim_poll = Parse.ToInt (data[3]),
            ...
            dataList.data.Add (fairyFood);
        }

        ZeroFormatterInitializer.Register ();

        dataPath += $"/{AssetDataFilename.fairyFood}";
        File.WriteAllBytes (dataPath, ZeroFormatterSerializer.Serialize (dataList));
        dataPath = $"{Application.streamingAssetsPath}/Data/Asset/FairyFoodData.json";
        File.WriteAllText (dataPath, JsonConvert.SerializeObject (dataList, Formatting.Indented));

        Debug.Log ($"<color=green>Asset data file \"{AssetDataFilename.fairyFood}\" has generated successful.</color>");
        Debug.Log (dataPath);
    }
}
