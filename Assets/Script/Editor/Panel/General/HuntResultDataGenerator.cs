...
public class HuntResultDataGenerator
{

    [MenuItem ("Data Generator/Panel/General/Hunt Result")]
    static void GenerateJsonFromData ()
    {
        string assetId = PanelDataFilename.huntResult.Split (".")[0].ToLower ();

        HuntResultPanelData panel = new HuntResultPanelData
        {
            asset_id = assetId,
            reward_material = new HuntResultPanelData.RewardMaterial
            {
                coin_id = "MAT1".ToLower (),
                reputation_id = "MAT2".ToLower ()
            },
            ui_sprite = new HuntResultPanelData.UiSprite
            {
                ...
                skip_button_background_id = "GUI41".ToLower (),
                skip_icon_id = "GUI40".ToLower (),
                skip_all_button_background_id = "GUI43".ToLower (),
                skip_all_icon_id = "GUI42".ToLower (),
                ...
                loot_summary_button_background_id = "GUI233".ToLower (),
                loot_summary_icon_id = "GUI209".ToLower (),
            ...
            text_visual = new HuntResultPanelData.TextVisual
            {
                hunt_slot = "255,255,255,255",
                skip_button = "255,255,255,255",
                skip_all_button = "255,255,255,255",
                summary = "255,255,255,255",
                rage = "255,255,255,255",
                loot_summary_button = "255,255,255,255",
        ...
        dataPath += $"/{PanelDataFilename.huntResult}";
        File.WriteAllBytes (dataPath, ZeroFormatterSerializer.Serialize (panel));
        dataPath = $"{Application.streamingAssetsPath}/Data/Panel/Hunt Result Panel Data.json";
        File.WriteAllText (dataPath, JsonConvert.SerializeObject (panel, Formatting.Indented));

        Debug.Log ($"<color=green>UI panel data file \"{PanelDataFilename.huntResult}\" has generated successful.</color>");
        Debug.Log (dataPath);
    }
}
