...
| 1 | new_game_intro | array of string (text) | "INT1", "INT2", "INT3", "INT4", "INT5", "INT6", "INT7", "INT8", "INT9" | The tutorial dialogue when you start the game. You can refer to the manual for changing the dialogue data [here](/MANUAL/Changing%20Dialogue%20Data.md). |
...
| 11 | fairy_rarity_experience_per_level | array of int (number) | 50, 60, 70, 85, 100 | The fairy experience need to gain for leveling up in each rarity. The first value is rarity 1 and the last value is rarity 5. You can add more values for rarity 6, 7 and so on if your fairy has more than rarity 5. Make sure the total value in this parameter is match with `fairy_rarity_max_level`. |
| 12 | fairy_rarity_max_level | array of int (number) | 100, 100, 100, 100, 100 | The maximun level each fairy can reach before it can awaken to another fairy in each rarity. The first value is rarity 1 and the last value is rarity 5. You can add more values for rarity 6, 7 and so on if your fairy has more than rarity 5. Make sure the total value in this parameter is match with `fairy_rarity_experience_per_level`. |
| 13 | fairy_experience_per_bug_catch | int (number) | 1 | The amount of experience gain when fairy catch a bug. |
...
| 19 | cage_unlock_cost | array of int (number) | 1000, 2500, 5000, 8000, 15000, 40000, 100000, 250000, 500000, 1000000, 1000000 | The cost for purchasing each cage. The first value is purchasing first cage, The second value is purchasing second cage, The last value is the extra cost adding to previos cost. In this example, the cost to purchase tenth cage is 1000000. The cost to purchase eleventh cage is the cost of tenth cage 1000000 + 1000000 = 2000000. The cost to purchase twelveth cage is the cost of eleventh cage, 2000000 + 1000000 = 3000000. |
| 20 | rage_experience_cap | array of int (number) | 100, 200, 300, 400, 500 | The accumulate rage amount for each rage level. In this default value, if rage value in 312, which is fall between 300 to 399, the rage level is 4. If rage value in 200, then the rage level is 3. Make sure this parameter has 5 values. |
...
| 22 | rage_up_message_id | string (text) | "HRM1" | The message display in hunt summary panel and hunt history panel when rage level is increased. Make sure this message ID is exist in [hunt result message language](/MANUAL/Changing Asset Language.md). |
| 23 | rage_down_message_id | string (text) | "HRM2" | The message display in hunt summary panel and hunt history panel when rage level is decreased. Make sure this message ID is exist in [hunt result message language](/MANUAL/Changing Asset Language.md). |
| 24 | inverse_rage_up_message_id | string (text) | "HRM8" | The message display in hunt summary panel and hunt history panel when rage level is increased with inverse rage effect active. Make sure this message ID is exist in [hunt result message language](/MANUAL/Changing Asset Language.md). |
| 25 | inverse_rage_down_message_id | string (text) | "HRM7" | The message display in hunt summary panel and hunt history panel when rage level is decreased with inverse rage effect or cold rage effect active. Make sure this message ID is exist in [hunt result message language](/MANUAL/Changing Asset Language.md). |
...
| 29 | max_reward_per_catch | int (number) | 6 | The maximun loot drop from bug if catch success. |
...
| 33 | profile_update_transition_timer | float (number) | 1.0 | The value and bar update transition time, in second, in top bar info panel after loading from loading assets phase, after close from hunt result panel and restore from backup save. |
...
| 36 | dialogue_text_display_speed | int (number) | 15 | The quantity of character dispkay every second in dialogue panels. |
...
| 39 | food_stall_next_event_after_hunt | int (number) | 50 | After this value of hunt, the food stall will end the sales or have a chance to start the sales depend on `food_stall_sales_chance`. |
...
| 45 | warehouse_full_claim_interval | int (number) | 1440 | The time, in minute, need to wait before full amount of items can claim in warehouse. |
| 46 | warehouse_next_claim_interval | int (number) | 360 | The time, in minute, need to wait before minimum amount, 1, of items can claim in warehouse. |
...
| 63 | ammo_dealer_next_event_after_hunt | int (number) | 250 | Trigger chance to get free ammo from ammo dealer after this value of hunt. |
